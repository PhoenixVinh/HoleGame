...
public class ShapeEditor : EditorWindow
{
    ...
    private void OnGUI()
    {
        GUILayout.Label("Circle Object Generator", EditorStyles.boldLabel);

        prefabToSpawn = (GameObject)EditorGUILayout.ObjectField("Prefab to Spawn", prefabToSpawn, typeof(GameObject), false);
        float newRadius = EditorGUILayout.Slider("Radius", radius, 0.1f, 20f); // Added slider with min/max
        if (newRadius != radius)
        {
            radius = newRadius;
            UpdatePreview();
    ...
    {
        if (prefabToSpawn == null || previewParent == null) return;
        ...
        Vector3 centerPosition = previewParent.transform.position;
        int numberOfObjects = Mathf.Max(1, Mathf.FloorToInt(2 * Mathf.PI * radius * 2f)); // Increased density for preview
    ...
    private void GenerateObjectsInCircle()
    {
        ...
        int numberOfObjects = Mathf.Max(1, Mathf.FloorToInt(2 * Mathf.PI * radius));
        ...
        Debug.Log($"Generated {numberOfObjects} objects in a circle with radius {radius}. Parent: {(parentObject != null ? parentObject.name : "None")}");
